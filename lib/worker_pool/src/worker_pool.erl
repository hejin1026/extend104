...
-export([start_link/0, submit/1, submit_async/1, idle/1,status/0]).
...
handle_call(status, _From, State = #state{available = Avail, pending = Pending}) ->
    {reply, [{avail, queue:len(Avail)},{pending, queue:len(Pending)}], State};
