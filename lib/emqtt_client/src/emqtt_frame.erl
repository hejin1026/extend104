...
-export([serialise/1, serialise_fixed/3]).
...
parse_frame(Bin, #mqtt_frame_fixed{ type = Type,
    ...
    case {Type, Bin} of
		{?CONNACK, <<FrameBin:Length/binary, Rest/binary>> } ->
			<<Reserved, Code>> = FrameBin,
			wrap(Fixed, #mqtt_frame_connack{return_code=Code}, Rest);
...
serialise_variable(#mqtt_frame_fixed { type = ?SUBSCRIBE } = Fixed,
			  	   #mqtt_frame_subscribe{ message_id = MsgId, topic_table = Topics},
				   <<>> = _PayloadBin) ->
					   
	PayloadBin = [ begin Len = length(Name), list_to_binary([<<Len:16/big>>, Name, <<0:6,QoS:2>>]) end
						|| #mqtt_topic{name = Name, qos = QoS}  <- Topics],				   
	?INFO("send info :~p", [PayloadBin]),
    serialise_fixed(Fixed, <<MsgId:16/big>>, list_to_binary(PayloadBin));
